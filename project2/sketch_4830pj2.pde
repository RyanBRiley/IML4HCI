...
//====== OSC SEND ======
void sendOsc() {
  //print("features.length: %d\n", features.length);
  OscMessage msg = new OscMessage("/wek/inputs");
  if (numFound > 0) {
    for (int i = 0; i < features.length; i++) {
      msg.add(features[i]);
      //print("Feauture ID : ",i, " value : ",features[i],"\n");
    }
  } else {
    for (int i = 0; i < features.length; i++) {
      msg.add(0.);
...
void sendInputNames() {
   ...
   String[] fingerNames = {"thumb", "index", "middle", "ring", "pinky", "second_thumb", "second_index", "2middle", "2ring", "2pinky"};
